...
constexpr std::coroutine_handle<To>
staticHandleCast(std::coroutine_handle<P> coroutine) {
...
struct SleepUntilPromise : co_async::RbTree<SleepUntilPromise>::RbNode,
                           Promise<void> {
...
struct Loop {
  co_async::RbTree<SleepUntilPromise> mRbTimer;
...
Task<int> hello1() {
  SPDLOG_DEBUG("hello开始睡1秒");
  co_await sleep_for(1s); // 1s 等价于 std::chrono::seconds(1)
  SPDLOG_DEBUG("hello1睡醒了");
  co_return 1;
}

Task<int> hello2() {
  SPDLOG_DEBUG("hello2开始睡2秒");
  co_await sleep_for(2s); // 2s 等价于 std::chrono::seconds(2)
  SPDLOG_DEBUG("hello2睡醒了");
  co_return 2;
}

Task<int> hello() {
  SPDLOG_DEBUG("hello开始等1和2");
  auto v = co_await when_any(hello1(), hello2());
  /* co_await hello1(); */
  /* co_await hello2(); */
  SPDLOG_DEBUG("hello看到{}睡醒了", (int)v.index() + 1);
...
int main(int argc, char *argv[]) {
  ...
  auto t = hello();
  getLoop().run(t);
  SPDLOG_DEBUG("主函数中得到hello结果: {}", t.mCoroutine.promise().result());
  return 0;
}
